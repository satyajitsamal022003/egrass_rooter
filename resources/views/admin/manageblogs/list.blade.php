                    ...
                    <form id="filter-form" method="get" class="form-horizontal">
                        <div class="col-md-3">
                            <select id="tag_id" name="tag_slug" data-rel="chosen" class="form-control col-md-7 col-xs-12">
                                ...
                                @foreach ($tags as $tag)
                                <option value="{{ $tag->slug }}" {{ request('tag_slug') == $tag->slug ? 'selected' : '' }}>
                                    {{ $tag->tag_title }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3">
                            <select id="cat_id" name="cat_slug" data-rel="chosen" class="form-control col-md-7 col-xs-12">
                                ...
                                <option value="{{ $category->slug }}" {{ request('cat_slug') == $category->slug ? 'selected' : '' }}>
                                    {{ $category->cat_title }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-4">
                            <input type="search" name="searchtxt" id="searchtxt" placeholder=" Search by Title" value="{{ request('searchtxt') }}" class="form-control">
                        </div>
                        <div class="col-sm-1">
                        ...
                        <table id="datatable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="status">
                                        <label class="switch">
                                            <input type="checkbox" onclick="catstatus({{ $blog->id }})" <?php if ($blog->is_active == '1') {
                                                                                                            echo 'checked';
                                                                                                        } ?>>
