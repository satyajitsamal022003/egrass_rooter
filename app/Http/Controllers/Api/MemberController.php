...
class MemberController extends Controller
{
    ...
    public function bulkcontactupload()
    {
        // Define the file path where the CSV is stored in the public directory
        $filePath = public_path('contactcsv/bulkcontact.csv');

        // Define the headers for CSV download
        $headers = [
            'Content-Type' => 'text/csv',
            'Content-Disposition' => 'attachment; filename="bulkcontact.csv"',
        ];

        // Return the file as a download response
        return response()->download($filePath, 'bulkcontact.csv', $headers);
    }
    ...
    public function bulkUpload(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'csv_file' => 'required|mimes:csv,txt|max:2048', // Limit file size to 2MB
        ]);
        ...
        while (($row = fgetcsv($handle, 1000, ',')) !== false) {
            ...
            $rowValidator = Validator::make($data, [
                'name' => 'required|string|max:255',
                'dob' => 'required|date',
                'phone_number' => 'required|unique:add_members,phone_number|max:15',
                'voters_id_number' => 'required|unique:add_members,voters_id_number|max:20',
                'code' => 'nullable|string|max:10',
                'occupation' => 'nullable|string|max:255',
                'email_id' => 'required|email|unique:add_members,email_id|max:255',
                'latitude' => 'nullable|numeric',
                'longitude' => 'nullable|numeric',
            ]);

            if ($rowValidator->fails()) {
                // Collect errors for each row that fails validation
                $errors[] = [
                    'row' => $data,
        ...
        if (count($errors) > 0) {
            return response()->json([
                'message' => 'CSV uploaded with some errors.',
                'success_count' => $successCount,
                'errors' => $errors,
            ], 207); // HTTP 207 means multi-status
        }

        return response()->json(['message' => 'All records successfully uploaded!'], 201);
    }
}
