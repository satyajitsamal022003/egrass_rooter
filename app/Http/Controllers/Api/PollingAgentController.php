...
class PollingAgentController extends Controller
{
    ...
    public function pollingagentemailvin(Request $request)
    {
        ...
        $emailpollingagents = PollingAgent::where('user_id', $userid)->where('role_type', 3)->where('email', $emailvin)
            ->get();
        ...
        if ($user->campaign_type == 1) {
            $pollingunits = Polling_unit::get();
        } else {
            $pollingunits = Polling_unit::where('state_id', $user->state)->get();
        }
        ...
        return response()->json([
            'message' => 'Polling agent email or vin retrieved successfully',
