...
class ApiAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $party = Party::where('id', $user->party_id)->first();
            ...
            return response()->json([
                ...
                'user_details' => [
                    ...
                    // 'candidate_image' => $party->candidate_image ? asset('images/parties/' . $party->candidate_image) : "",
                ],
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
    ...
    public function getProfile($id)
    {
        ...
        $allpoliticalparty = Party::select('id', 'party_name')->where('is_active', 1)->get();

        // Format dates using Carbon
        $formattedDateOfRegistration = Carbon::parse($user->date_of_registration)->format('Y-m-d');
        $formattedDob = Carbon::parse($user->dob)->format('Y-m-d');
        ...
        $profile = [
            ...
            'registration_date' => $formattedDateOfRegistration, // Formatted Registration Date
            'political_party' => $user->political_party,
            'allparty' => $allpoliticalparty
        ];
    ...
    public function verifyToken(Request $request)
    {
        try {
            ...
            $userofid = $request->get('token');

            // Verify the token
            if ($userid == $userofid) {
                return response()->json(['status' => 200, 'message' => 'Token is valid.'], 200);
            } else {
                return response()->json(['status' => 401, 'error' => 'Token does not match.'], 401);
            }
        } catch (JWTException $e) {
            return response()->json(['status' => 401, 'error' => 'Token is invalid or expired.'], 401);
        }
    }

    public function homepagelayout()
    {
        // Fetching the 'About Us' content
        $aboutus = Site_content::where('id', 4)->first();

        // Fetching active quicklinks
        $quicklinks = Page::where('is_active', 1)->get();

        // Fetching Site settings data
        $siteSettings = Sitesetting::first(); // Assuming you want the first record

        // Map the quicklinks to only include the page_name column
        $quicklinksArray = $quicklinks->map(function ($page) {
            return $page->page_name;
        });

        // Prepare the layout data including additional site settings data
        $layout_data = [
            'about_us' => $aboutus->description,
            'quicklinks' => $quicklinksArray,
            'mail_id' => $siteSettings->admin_mail_id,
            'location_address' => $siteSettings->location,
            'contact_number' => $siteSettings->contact,
            'logo' => $siteSettings->logo ? asset('images/' . $siteSettings->logo) : '',
        ];

        return response()->json($layout_data); // Return the data as a JSON response
    }
}
