...
class EventController extends Controller
{
    public function index()
    {
        ...
        if (!$events->isEmpty()) {
            foreach ($events as $key => $event) {
                ...
                $event_data[$key]['is_active'] = $event->is_active ?? "";
                $event_data[$key]['event_time'] = isset($event->event_time) ? date('h:i', strtotime($event->event_time)) : "";
    ...
    public function update(Request $request)
    {
        ...
        $eventDate = \DateTime::createFromFormat('Y-m-d', $request->input('event_date'));
        if (!$eventDate) {
            return response()->json(['message' => 'Invalid date format. Expected format is YYYY/mm/dd'], 422);
        }
        $formattedEventDate = $eventDate->format('Y-m-d');
